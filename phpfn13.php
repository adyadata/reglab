...
function CurrentProjectID() {
	...
	return "{BDF410E2-F8A1-44BF-83FA-BA321C2274F7}";
}
...
class cAdvancedSecurity {
	...
	function SetUpUserLevelEx() {
		...
		if (count($this->UserLevelPriv) == 0 && $this->IsAdmin() && $Page != NULL && @$_SESSION[EW_SESSION_USER_LEVEL_MSG] == "") {
			...
			$Page->Page_Terminate("t_user_levelslist.php");
		}
		return TRUE;
	}
...
function Database_Connecting(&$info) {
	...
	if (ew_CurrentUserIP () == "127.0.0.1"  || ew_CurrentUserIP () == ":: 1"  || ew_CurrentHost () == "localhost" ) { // testing on local PC
		...
		$info["db"] = "db_reglab"; // sesuaikan dengan nama database di komputer localhost
	} elseif (ew_CurrentHost () == "reglab.nma-indonesia.com") { // setting koneksi database untuk komputer server
		$info["host"] = "mysql.idhostinger.com";  // sesuaikan dengan ip address atau hostname komputer server
		$info["user"] = "u945388674_regla"; // sesuaikan dengan username database di komputer server
		$info["pass"] = "M457r1P 81"; // sesuaikan deengan password database di komputer server
		$info["db"] = "u945388674_regla"; // sesuaikan dengan nama database di komputer server
	}
}
