...
$t_daftard_delete = NULL; // Initialize page object first

class ct_daftard_delete extends ct_daftard {
	...
	// Page object name
	var $PageObjName = 't_daftard_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("t_daftardlist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in t_daftard class, t_daftardinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("t_daftardlist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($t_daftard_delete)) $t_daftard_delete = new ct_daftard_delete();

// Page init
$t_daftard_delete->Page_Init();

// Page main
$t_daftard_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t_daftard_delete->Page_Render();
?>
...
var CurrentForm = ft_daftarddelete = new ew_Form("ft_daftarddelete", "delete");

// Form_CustomValidate event
ft_daftarddelete.Form_CustomValidate = 
...
ft_daftarddelete.ValidateRequired = true;
<?php } else { ?>
ft_daftarddelete.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ft_daftarddelete.Lists["x_PraktikumID"] = {"LinkField":"x_PraktikumID","Ajax":true,"AutoFill":false,"DisplayFields":["x_Nama","x_Biaya","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"t_praktikum"};
...
<?php $t_daftard_delete->ShowPageHeader(); ?>
<?php
$t_daftard_delete->ShowMessage();
?>
<form name="ft_daftarddelete" id="ft_daftarddelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t_daftard_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t_daftard_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="t_daftard">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($t_daftard_delete->RecKeys as $key) { ?>
...
<?php if ($t_daftard->Tgl->Visible) { // Tgl ?>
		<th><span id="elh_t_daftard_Tgl" class="t_daftard_Tgl"><?php echo $t_daftard->Tgl->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$t_daftard_delete->RecCnt = 0;
$i = 0;
while (!$t_daftard_delete->Recordset->EOF) {
	$t_daftard_delete->RecCnt++;
	$t_daftard_delete->RowCnt++;

	// Set row properties
	$t_daftard->ResetAttrs();
	$t_daftard->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$t_daftard_delete->LoadRowValues($t_daftard_delete->Recordset);
...
<?php } ?>
<?php if ($t_daftard->Tgl->Visible) { // Tgl ?>
		<td<?php echo $t_daftard->Tgl->CellAttributes() ?>>
<span id="el<?php echo $t_daftard_delete->RowCnt ?>_t_daftard_Tgl" class="t_daftard_Tgl">
...
<?php
	$t_daftard_delete->Recordset->MoveNext();
}
$t_daftard_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $t_daftard_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
ft_daftarddelete.Init();
</script>
<?php
$t_daftard_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$t_daftard_delete->Page_Terminate();
?>
