...
$t_daftard_grid = NULL; // Initialize page object first

class ct_daftard_grid extends ct_daftard {
	...
	// Page object name
	var $PageObjName = 't_daftard_grid';

	// Grid form hidden field names
	var $FormName = 'ft_daftardgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["t_daftard"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->DaftradID->CurrentValue = strval($arKeys[0]); // DaftradID
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "t_daftarm") {
			$this->DaftarmID->Visible = FALSE;
			if ($GLOBALS["t_daftarm"]->EventCancelled) $this->EventCancelled = TRUE;
		}
