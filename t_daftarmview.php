...
$t_daftarm_view = NULL; // Initialize page object first

class ct_daftarm_view extends ct_daftarm {
	...
	// Page object name
	var $PageObjName = 't_daftarm_view';
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["DaftarmID"] <> "") {
			$this->RecKey["DaftarmID"] = $_GET["DaftarmID"];
			$KeyUrl .= "&amp;DaftarmID=" . urlencode($this->RecKey["DaftarmID"]);
		}
	...
	function Page_Init() {
		...
		if (@$_GET["DaftarmID"] <> "") {
			...
			$gsExportFile .= ew_StripSlashes($_GET["DaftarmID"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			} elseif (@$_POST["DaftarmID"] <> "") {
				$this->DaftarmID->setFormValue($_POST["DaftarmID"]);
				$this->RecKey["DaftarmID"] = $this->DaftarmID->FormValue;
			} else {
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "t_daftarmlist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "t_daftarmlist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupOtherOptions() {
		...
		// "detail_t_daftard"
		$item = &$option->Add("detail_t_daftard");
		$body = $Language->Phrase("ViewPageDetailLink") . $Language->TablePhrase("t_daftard", "TblCaption");
		...
		$links = "";
		if ($GLOBALS["t_daftard_grid"] && $GLOBALS["t_daftard_grid"]->DetailView && $Security->CanView() && $Security->AllowView(CurrentProjectID() . 't_daftard')) {
		...
		if ($GLOBALS["t_daftard_grid"] && $GLOBALS["t_daftard_grid"]->DetailEdit && $Security->CanEdit() && $Security->AllowEdit(CurrentProjectID() . 't_daftard')) {
		...
		if ($GLOBALS["t_daftard_grid"] && $GLOBALS["t_daftard_grid"]->DetailAdd && $Security->CanAdd() && $Security->AllowAdd(CurrentProjectID() . 't_daftard')) {
		...
		$item->Body = $body;
		$item->Visible = $Security->AllowList(CurrentProjectID() . 't_daftard');
		if ($item->Visible) {
	...
	function ExportData() {
		...
		// Export detail records (t_daftard)
		if (EW_EXPORT_DETAIL_RECORDS && in_array("t_daftard", explode(",", $this->getCurrentDetailTable()))) {
			global $t_daftard;
			if (!isset($t_daftard)) $t_daftard = new ct_daftard;
			$rsdetail = $t_daftard->LoadRs($t_daftard->GetDetailFilter()); // Load detail records
			if ($rsdetail && !$rsdetail->EOF) {
				...
				if ($this->Export <> "csv" || EW_EXPORT_DETAIL_RECORDS_FOR_CSV) {
					...
					$t_daftard->ExportDocument($Doc, $rsdetail, 1, $detailcnt);
				}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("t_daftard", $DetailTblVar)) {
				...
				if ($GLOBALS["t_daftard_grid"]->DetailView) {
					$GLOBALS["t_daftard_grid"]->CurrentMode = "view";
...
// Create page object
if (!isset($t_daftarm_view)) $t_daftarm_view = new ct_daftarm_view();

// Page init
$t_daftarm_view->Page_Init();

// Page main
$t_daftarm_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t_daftarm_view->Page_Render();
?>
...
var CurrentForm = ft_daftarmview = new ew_Form("ft_daftarmview", "view");

// Form_CustomValidate event
ft_daftarmview.Form_CustomValidate = 
...
ft_daftarmview.ValidateRequired = true;
<?php } else { ?>
ft_daftarmview.ValidateRequired = false; 
<?php } ?>
...
<?php if ($t_daftarm->Export == "") { ?>
<div class="ewToolbar">
<?php if (!$t_daftarm_view->IsModal) { ?>
<?php if ($t_daftarm->Export == "") { ?>
<?php $Breadcrumb->Render(); ?>
<?php } ?>
<?php } ?>
<?php $t_daftarm_view->ExportOptions->Render("body") ?>
<?php
	foreach ($t_daftarm_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if (!$t_daftarm_view->IsModal) { ?>
<?php if ($t_daftarm->Export == "") { ?>
...
<?php } ?>
<?php $t_daftarm_view->ShowPageHeader(); ?>
<?php
$t_daftarm_view->ShowMessage();
?>
<?php if (!$t_daftarm_view->IsModal) { ?>
<?php if ($t_daftarm->Export == "") { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($t_daftarm_view->Pager)) $t_daftarm_view->Pager = new cPrevNextPager($t_daftarm_view->StartRec, $t_daftarm_view->DisplayRecs, $t_daftarm_view->TotalRecs) ?>
<?php if ($t_daftarm_view->Pager->RecordCount > 0 && $t_daftarm_view->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($t_daftarm_view->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $t_daftarm_view->PageUrl() ?>start=<?php echo $t_daftarm_view->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($t_daftarm_view->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $t_daftarm_view->PageUrl() ?>start=<?php echo $t_daftarm_view->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $t_daftarm_view->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($t_daftarm_view->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $t_daftarm_view->PageUrl() ?>start=<?php echo $t_daftarm_view->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($t_daftarm_view->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $t_daftarm_view->PageUrl() ?>start=<?php echo $t_daftarm_view->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $t_daftarm_view->Pager->PageCount ?></span>
</div>
<?php } ?>
<div class="clearfix"></div>
</form>
<?php } ?>
<?php } ?>
<form name="ft_daftarmview" id="ft_daftarmview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t_daftarm_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t_daftarm_view->Token ?>">
<?php } ?>
...
<?php if ($t_daftarm->_UserID->Visible) { // UserID ?>
	<tr id="r__UserID">
		<td><span id="elh_t_daftarm__UserID"><?php echo $t_daftarm->_UserID->FldCaption() ?></span></td>
...
<?php echo $t_daftarm->_UserID->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($t_daftarm->TglJam->Visible) { // TglJam ?>
	<tr id="r_TglJam">
		<td><span id="elh_t_daftarm_TglJam"><?php echo $t_daftarm->TglJam->FldCaption() ?></span></td>
...
<?php echo $t_daftarm->TglJam->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php if ($t_daftarm->JumlahBayar->Visible) { // JumlahBayar ?>
	<tr id="r_JumlahBayar">
		<td><span id="elh_t_daftarm_JumlahBayar"><?php echo $t_daftarm->JumlahBayar->FldCaption() ?></span></td>
...
<?php echo $t_daftarm->JumlahBayar->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php } ?>
<?php
	if (in_array("t_daftard", explode(",", $t_daftarm->getCurrentDetailTable())) && $t_daftard->DetailView) {
?>
