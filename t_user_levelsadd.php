...
$t_user_levels_add = NULL; // Initialize page object first

class ct_user_levels_add extends ct_user_levels {
	...
	// Page object name
	var $PageObjName = 't_user_levels_add';
		...
		switch ($this->CurrentAction) {
			...
			case "C": // Copy an existing record
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("t_user_levelslist.php"); // No matching record, return to list
				}
				break;
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "t_user_levelsview.php")
...
// Create page object
if (!isset($t_user_levels_add)) $t_user_levels_add = new ct_user_levels_add();

// Page init
$t_user_levels_add->Page_Init();

// Page main
$t_user_levels_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t_user_levels_add->Page_Render();
?>
...
var CurrentForm = ft_user_levelsadd = new ew_Form("ft_user_levelsadd", "add");

// Validate form
ft_user_levelsadd.Validate = function() {
...
ft_user_levelsadd.Form_CustomValidate = 
...
ft_user_levelsadd.ValidateRequired = true;
<?php } else { ?>
ft_user_levelsadd.ValidateRequired = false; 
<?php } ?>
...
<?php if (!$t_user_levels_add->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $t_user_levels_add->ShowPageHeader(); ?>
<?php
$t_user_levels_add->ShowMessage();
?>
<form name="ft_user_levelsadd" id="ft_user_levelsadd" class="<?php echo $t_user_levels_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t_user_levels_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t_user_levels_add->Token ?>">
<?php } ?>
...
<input type="text" data-table="t_user_levels" data-field="x_userlevelid" name="x_userlevelid" id="x_userlevelid" size="30" placeholder="<?php echo ew_HtmlEncode($t_user_levels->userlevelid->getPlaceHolder()) ?>" value="<?php echo $t_user_levels->userlevelid->EditValue ?>"<?php echo $t_user_levels->userlevelid->EditAttributes() ?>>
</span>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $t_user_levels_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
ft_user_levelsadd.Init();
</script>
<?php
$t_user_levels_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$t_user_levels_add->Page_Terminate();
?>
